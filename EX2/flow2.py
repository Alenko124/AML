...
class MaskedCouplingLayer(nn.Module):
    ...
    def forward(self, z):
        """
        Transform a batch of data through the coupling layer (from base to data).
        ...
        x = z_masked + (1 - mask) * (z * torch.exp(s) + t)

        # log determinant Jacobiana
...
def train(model, optimizer, data_loader, epochs, device):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (x, _) in enumerate(progress_bar):
            ...
            # 🔥 DEBUG SAMO PRVI BATCH
            if batch_idx == 0:
                with torch.no_grad():
                    z, log_det = model.inverse(x)
                    base_lp = model.base().log_prob(z).mean()
                    total_lp = (base_lp + log_det.mean())

                    print("\n--- DEBUG ---")
                    print("base log_prob:", base_lp.item())
                    print("log_det:", log_det.mean().item())
                    print("total log_prob:", total_lp.item())
                    print("-------------\n")
            optimizer.step()
            z, log_det = model.inverse(x)

            print("base log_prob:", model.base().log_prob(z).mean().item())
            print("log_det:", log_det.mean().item())
            print("total log_prob:", (model.base().log_prob(z) + log_det).mean().item())
            with torch.no_grad():
